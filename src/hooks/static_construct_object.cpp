...
auto __cdecl static_construct_object_hook::trampoline(UClass* object_class, UObject* outer, FName name, DWORD object_flags, UObject* template_, void* error, UObject* sub_object_root, void* graph, void* extra_, void* extra__) -> UObject* {
#if LOG_SCO
  LOG("StaticConstructObject({}, {})", object_class != nullptr ? object_class->GetName() : "nullptr", outer != nullptr ? outer->GetName() : "nullptr");
#endif

  return static_construct_object_hook::instance()->hook_.original()(object_class, outer, name, object_flags, template_, error, sub_object_root, graph, extra_, extra__);
}